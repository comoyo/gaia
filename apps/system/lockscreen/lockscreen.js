...
var LockScreen = {
  ...
  /*
   * If user had released the finger and the handler had already
  ...
  init: function ls_init() {
    ...
    window.parent.LockScreen.lockIfEnabled(true);
    ...
    if (window.parent.IccHelper.enabled) {
      window.parent.IccHelper.addEventListener('cardstatechange', this);
      window.parent.IccHelper.addEventListener('iccinfochange', this);
    }
    ...
    SettingsListener.observe(
      'accessibility.screenreader', false, function(value) {
    ...
    var wallpaperURL = new window.parent.SettingsURL();

    SettingsListener.observe('wallpaper.image',
      '../resources/images/backgrounds/default.png', function(value) {
    ...
    function onLockChange() {
      window.parent.LockScreen.locked = self.locked;
    }
  ...
  setEnabled: function ls_setEnabled(val) {
    ...
    window.parent.LockScreen.enabled = this.enabled;
    ...
    [this.slideLeft, this.slideRight, this.slideCenter]
      ...
      if (easing) {
        ...
        var tsEnd = function ls_tsEnd(evt) {
          ...
          // when it bounce back.
          //
          // This is because crossing the origin will restore
          // it as well,
          // but we don't need to reset the blue are at such
          // scenario.
    ...
    switch (target) {
      ...
      case this.altCamera:
        var panelOrFullApp = function panelOrFullApp() {
          ...
          self.unlock( /* instant */ null, /* detail */ {
            areaCamera: true
          });
    ...
    switch (panel) {
      ...
      case 'camera':
        ...
        frame.onload = function cameraLoaded() {
          document.body.classList.add('lockscreen-camera');
  ...
  unloadPanel: function ls_unloadPanel(panel, toPanel, callback) {
    ...
    switch (panel) {
      ...
      case 'camera':
        document.body.classList.remove('lockscreen-camera');
        break;
      ...
      default:
        ...
        overlay.addEventListener(
          'transitionend', function ls_unloadDefaultPanel(evt) {
          if (evt.target !== this)
            return;
  ...
  switchPanel: function ls_switchPanel(panel) {
    ...
    this.loadPanel(panel, function panelLoaded() {
      self.unloadPanel(overlay.dataset.panel, panel, function panelUnloaded() {
